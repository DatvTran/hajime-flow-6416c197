...
if [[ -n "$SKIP_PROD" ]]; then
  ...
  if [[ "$HTTP_CODE" == "200" ]]; then
    ...
    # The live handler (server/index.mjs) reports database:"up" — see IMPLEMENTATION.md.
...
# Bounded to the array's own [ ... ]; block — an unbounded `grep -A N` window
# bleeds into the next array (ADMIN_ASSIGNABLE_ROLES, which *does* list them
# by design) and falsely trips this stop condition.
PRIV_IN_SELF_REG=$(awk '/const SELF_REGISTERABLE_ROLES/{flag=1} flag{print; if (/\];/) exit}' \
