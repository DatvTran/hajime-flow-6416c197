...
# Scope the grep to just the array literal (marker through its closing "];"),
# so it doesn't bleed into unrelated constants defined further down the file.
PRIV_IN_SELF_REG=$(awk '/SELF_REGISTERABLE_ROLES *= *\[/{flag=1} flag{print} flag && /\];/{exit}' \
