...
if [[ -n "$SKIP_PROD" ]]; then
  ...
  if [[ "$HTTP_CODE" == "200" ]]; then
    ...
    if [[ "$OK_VAL" == "true" && ( "$DB_VAL" == "connected" || "$DB_VAL" == "up" ) ]]; then
      pass "Health 200 OK — ok:true · database:$DB_VAL"
    else
      fail "Health 200 but ok=$OK_VAL database=$DB_VAL"
      stop "Health endpoint returned ok=$OK_VAL database=$DB_VAL. Expected ok:true + database:connected/up."
    fi
  else
...
# Extract only the SELF_REGISTERABLE_ROLES array body (up to the closing ]; line)
# and check whether privileged roles appear inside it.
PRIV_IN_SELF_REG=$(awk '/SELF_REGISTERABLE_ROLES\s*=\s*\[/{found=1} found{print} /\];/{if(found) exit}' \
